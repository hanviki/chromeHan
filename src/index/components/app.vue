<template>
  <div class="content_page">
    <div class="content_page_main">
	  ...
	  <a-checkbox :checked="checkedOne" @change="onChangeOne"> 正1 </a-checkbox>
    <a-checkbox :checked="checkedTwo" @change="onChangeTwo"> 最新赔率 </a-checkbox>
...
export default {
  ...
  methods: {
    ...
    getAllData() {
      ...
      let flagOne = this.checkedOne==true?'1':'-1';
       let flagTwo = this.checkedTwo==true?6:3;
      let stime = 0;
      $("#data_main_content tr").each(function () {
        ...
        if ($(this).attr("id") != "filterTips") {
         ...
         // var title = $(this).find("td").eq(2).attr("title"); //开赛时间
          ...
          var flagSum = $(this).find("td").eq(2).find("span").html(); //是否1或-1
          console.info(flagSum)
          var zhu = parseFloat($(this).find("td").eq(flagTwo).find("a").eq(0).find("span").html()); //初赔客队
          var avg = parseFloat($(this).find("td").eq(flagTwo+1).find("a").eq(0).find("span").html()); //初赔平赔
          var ke = parseFloat($(this).find("td").eq(flagTwo+2).find("a").eq(0).find("span").html()); //初赔客队

          // console.info(bocai+","+title+",zhu:"+zhu+",avg"+avg+",ke:"+ke);
          if (flag == true) {
            if (zhu > ke && flagSum== flagOne) {
